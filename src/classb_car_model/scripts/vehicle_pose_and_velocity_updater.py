...
class vehicle_pose_and_velocity_updater:
	def __init__(self):
		...
		self.front_pose_pub = rospy.Publisher('/classb_car/front_pose', PoseStamped, queue_size = 1)
		self.rear_pose_pub = rospy.Publisher('/classb_car/rear_pose', PoseStamped, queue_size = 1)
		self.center_pose_pub = rospy.Publisher('/classb_car/center_pose', PoseStamped, queue_size = 1)
		self.vel_pub = rospy.Publisher('/classb_car/velocity', TwistStamped, queue_size = 1)
	...
	def model_cb(self,data):
		try:
			vehicle_model_index = data.name.index("classb_car")
		except:
			return
		...
		center_x = vehicle_position.position.x
		center_y = vehicle_position.position.y
		front_x = center_x + cos(yaw) * 0.945
		front_y = center_y + sin(yaw) * 0.945
