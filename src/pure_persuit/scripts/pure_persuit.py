...
class PurePersuit:
	def __init__(self):
		...
		rospy.Subscriber('/classb_car/rear_pose', PoseStamped, self.pose_cb, queue_size = 1)
		rospy.Subscriber('/classb_car/velocity', TwistStamped, self.vel_cb, queue_size = 1)
		rospy.Subscriber('/final_waypoints', Lane, self.lane_cb, queue_size = 1)

		self.twist_pub = rospy.Publisher('/classb_car/cmd_vel', Twist, queue_size = 1)
	...
	def loop(self):
		...
		while not rospy.is_shutdown():
			if self.currentPose and self.currentVelocity and self.currentWaypoints:
				...
				if count_num < 1:
					rospy.logwarn("I'am in the if loop")
